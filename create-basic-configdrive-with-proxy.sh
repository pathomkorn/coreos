...
USAGE="Usage: $0 -H HOSTNAME -S SSH_FILE -1 INTERFACE -2 IPADDR/PREFIX -3 GATEWAY -4 DNSSERVER -5 PROXY [-p /target/path] [-d|-e|-i|-n|-t|-h]
Options:
    ...
    -5 http://IP:PORT  Internet proxy server.
...
while getopts "1:2:3:4:5:d:e:H:i:n:p:S:t:l:u:h" OPTION
do
    case $OPTION in
...
if [ -z "$ETCD_PROXY" ]; then
    echo "$0: The Internet proxy server parameter '-5' is required." >&2
    exit 1
fi
...
CLOUD_CONFIG="${CLOUD_CONFIG/<ETCD_PROXY>/${ETCD_PROXY}}"
